...
class KarelStateGenerator(object):
    ...
    def generate_single_state_shelf_stocker(self, h=8, w=8, wall_prob=0.1):
        ...
        valid programs:
            DEF run m( WHILE c( frontIsClear c) w( pickMarker turnLeft move putMarker move move turnLeft move IF c( frontIsClear c) i( move i) w) m)
            DEF run m( WHILE c( frontIsClear c) w( pickMarker turnLeft move putMarker turnRight turnRight move turnLeft move IF c( frontIsClear c) i( move i) w) m)
            DEF run m( WHILE c( frontIsClear c) w( pickMarker turnLeft move putMarker turnLeft turnLeft move turnLeft move IF c( frontIsClear c) i( move i) w) m)
        ...
        # put markers at row h-1, but at alternate locations
        #self.print_state(s)
        ...
        # place walls at h - 3 and alternate locations at h - 2
        s[h - 4, :, 4] = True
        ...
        valid_agent_pos += [(h-3, c) for c in range(1, w-1, 2)]
        agent_valid_positions = list(set(valid_agent_pos))
        # generate valid marker positions
        expected_marker_positions = [(h-3, c) for c in range(1, w-1, 2)]
        expected_empty_marker_positions = [(h-2, c) for c in range(1, w-1, 2)]
        metadata = {'agent_valid_positions':agent_valid_positions,
                    'expected_marker_positions':expected_marker_positions,
                    'expected_empty_marker_positions': expected_empty_marker_positions}
    ...
    def generate_single_state_chain_smoker(self, h=8, w=8, wall_prob=0.1, is_top_off=False):
        ...
        # NOTEEEEEEEEEEEEEEEEEEEEEE: need marker in last position as the condition is to check till I reach end
    ...
    def generate_single_state_place_setter(self, h=8, w=8, wall_prob=0.1):
        ...
        Valid programs:
        DEF run m( WHILE c( frontIsClear c) w( IFELSE c( leftIsClear c) i( move i) ELSE e( putMarker move e) w) m)
        DEF run m( WHILE c( frontIsClear c) w( IF c( not c( leftIsClear c) c) i( putMarker i) move  w) m)
        ...
        #self.print_state(s)
        s[h-3, 1:w-2, 4] = self.rng.rand(w-3) > 1 - wall_prob
        #self.print_state(s)

        # place 0 markers every where
        #num_walls = np.sum(s[h - 3, 1:w-1, 4])
        s[:, :, 5] = True
        #s[agent_pos[0], agent_pos[1], 5] = False
        #s[agent_pos[0], agent_pos[1], 5 + num_walls] = True
        #self.print_state(s)
        ...
        agent_valid_positions = list(set(valid_agent_pos))
        # generate valid marker positions
        expected_marker_positions = [(h-2, c) for c in range(1, w-1) if s[h-3, c, 4]]
        not_expected_marker_positions = [(h - 2, c) for c in range(1, w - 1) if not s[h - 3, c, 4]]
        metadata = {'agent_valid_positions': agent_valid_positions,
                    'expected_marker_positions': expected_marker_positions,
                    'not_expected_marker_positions': not_expected_marker_positions}
    ...
    def generate_single_state_find_marker(self, h=13, w=13, wall_prob=0.1):
        ...
        # Karel initial location
        # FIXME: commented following block of code to fix karel agent position to bottom left corner
        #valid_loc = False
        #while (not valid_loc):
        #    y = self.rng.randint(0, h)
        #    x = self.rng.randint(0, w)
        ...
        #        s[y, x, self.rng.randint(0, 4)] = True
        #        assert not s[y, x, 4]
        assert not s[h-2, 1, 4]
        y, x = h-2, 1
        s[h-2, 1, self.rng.randint(0, 4)] = True

        # Marker location
        # FIXME: commented following block of code to fix marker position at farthest away location in grid
        #valid_loc = False
        #while (not valid_loc):
        #    ym = self.rng.randint(0, h)
        #    xm = self.rng.randint(0, w)
        ...
        assert np.sum(s[:, :, 7:]) == 0
        metadata = {'agent_valid_positions': None}
        return s, y, x, np.sum(s[:, :, 4]), metadata
