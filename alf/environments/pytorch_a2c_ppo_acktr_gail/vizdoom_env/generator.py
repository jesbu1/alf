...
import sys
sys.path.insert(0, '.')
sys.path.insert(0, './vizdoom_env/')
sys.path.insert(0, './vizdoom_env/vdsl/')
...
from vizdoom_env_wrapper import Vizdoom_env
from vdsl.dsl_parse_and_trace import parse_and_trace as vizdoom_parse_and_trace
from vdsl.random_code_generator import DoomProgramGenerator
from vdsl.vocab import VizDoomDSLVocab
...
def generator(config):
    ...
    while True:
        ...
        exe, compile_success, record_dict = vizdoom_parse_and_trace(program)
        ...
        for k, world in enumerate(world_list[config.num_demo_per_program:]):
            ...
            if not success or len(world.s_h) < config.min_demo_length \
                ...
                if len(world.s_h) < config.min_demo_length and exec_ratio is not None and len(world.s_h) == 1:
                    all_success = True
                else:
                    all_success = False
                    break
        if all_success and config.cover_all_branches_in_demos:
            exec_ratio = _branch_execution_ratio(record_dict)
            if exec_ratio is None or exec_ratio >= 1.0:
                all_success = True
            else:
                all_success = False
        ...
        #num_trail = 0
        #num_err_trial = 0
        #while cur_num_demo < num_demo and num_trial < config.max_demo_generation_trial:
        #    world_list[0]
        ########
...
def main():
    ...
    parser.add_argument('--num_train', type=int, default=35000, help=' ')
    parser.add_argument('--num_test', type=int, default=7500, help=' ')
    parser.add_argument('--num_val', type=int, default=7500, help=' ')
    parser.add_argument('--seed', type=int, default=123, help=' ')
    parser.add_argument('--max_program_length', type=int, default=45)
    parser.add_argument('--min_demo_length', type=int, default=2)
    parser.add_argument('--max_demo_length', type=int, default=20, help=' ')
    parser.add_argument('--num_demo_per_program', type=int, default=10, help=' ')
    parser.add_argument('--num_test_demo_per_program', type=int, default=10, help=' ')
    parser.add_argument('--width', type=int, default=80)
    parser.add_argument('--height', type=int, default=80)
    parser.add_argument('--max_demo_generation_trial', type=int, default=200)
